...
if [ $? -eq 0 ]; then
    ...
    if echo "$output" | grep "$WAS_BASE_VERSION_ENTITLED"; then
        echo "An entitled IBM account is NOT acceptable, as we're going to install a trial version now."
        ...
        echo "A non-entitled IBM account is accepted, and we're going to install a trial version now."
    fi
else
